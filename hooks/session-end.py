...
def main() -> None:
    ...
    raw_session_id = hook_input.get("session_id", "unknown")
    # Sanitize: session_id is composed into a filename below — drop any path-traversal chars.
    session_id = re.sub(r"[^a-zA-Z0-9_-]", "_", str(raw_session_id))[:64] or "unknown"
    ...
    # hook runs with stripped PATH — must use absolute uv path
    # Configure via UV_BIN env var or edit config.py; see README "Configuration"
    import os as _os
    UV_BIN = _os.environ.get("UV_BIN", str(Path.home() / ".local" / "bin" / "uv"))
    cmd = [
        UV_BIN,
        "run",
        "--directory",
    ...
    try:
        ...
        # Don't leave the temp context file behind if the spawn never happened —
        # flush.py is what normally cleans it up.
